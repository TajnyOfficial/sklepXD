...
const DEMO = [
  { id: '1', number: 'MM/2026/03/001', from: 'Sektor A', to: 'Sektor C', status: 'completed', items: [{name: 'Śruba M8x40', qty: 200}], date: '2026-03-11', created_by: 'Tomasz Lewandowski' },
  { id: '2', number: 'MM/2026/03/002', from: 'Sektor B', to: 'Sektor D', status: 'in_transit', items: [{name: 'Farba akrylowa biała 10L', qty: 10}, {name: 'Pędzel płaski 75mm', qty: 15}], date: '2026-03-12', created_by: 'Maria Zielińska' },
];
...
export default function TransfersPage() {
  ...
  function handleSave() {
    ...
    const tr = { id: crypto.randomUUID(), number: `MM/2026/03/${String(transfers.length + 1).padStart(3, '0')}`, from: form.from, to: form.to, status: 'in_transit', items: validItems.map(i => ({ name: i.product_name, qty: parseInt(i.qty) })), date: new Date().toISOString().split('T')[0], created_by: 'Bieżący użytkownik' };
  ...
  function completeTransfer(id) { setTransfers(prev => prev.map(t => t.id === id ? { ...t, status: 'completed' } : t)); toast.success('Przesunięcie zakończone'); }
  ...
  return (
    <div className="page animate-fadeIn">
      ...
      <div className="table-container">
        <table>
          <thead><tr><th>Nr dokumentu</th><th>Z</th><th>Do</th><th>Pozycje</th><th>Data</th><th>Status</th><th></th></tr></thead>
          <tbody>
            {transfers.map(t => (
              <tr key={t.id}>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Nowe przesunięcie MM" size="modal-lg" footer={<><button className="btn btn-secondary" onClick={() => setShowModal(false)}>Anuluj</button><button className="btn btn-primary" onClick={handleSave}>Utwórz</button></>}>
        <div className="input-row mb-16"><div className="input-group"><label>Z lokalizacji *</label><input className="input" value={form.from} onChange={e => setForm(p => ({ ...p, from: e.target.value }))} placeholder="np. Sektor A" /></div><div className="input-group"><label>Do lokalizacji *</label><input className="input" value={form.to} onChange={e => setForm(p => ({ ...p, to: e.target.value }))} placeholder="np. Sektor C" /></div></div>

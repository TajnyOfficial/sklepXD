...
import { FiPlus, FiDownload, FiEye, FiMail, FiEdit, FiCornerDownRight, FiCheckCircle } from 'react-icons/fi';import Modal from '../../components/Modal';
...
export default function InvoicesPage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if(error) throw error;
      if(data && data[0]) setInvoices(prev => [data[0], ...prev]);
      toast.success(`Zapisano fakturę`);
      setShowModal(false);
      setForm(EMPTY);
    } catch (error) { 
      if (error.message.includes('note')) toast.error('Brak kolumny "note" w bazie. Odpal skrypt SQL!');
  ...
  async function handleSavePayment() {
    ...
    const expected = getEffectiveGross(paymentTarget);
    const paid = getPaidAmount(paymentTarget);
    
    let amountToSave = rawAmount;
    if (paid > expected + 0.01 && rawAmount > 0) {
      amountToSave = -rawAmount;
    }

    const newPayment = {
      document_id: paymentTarget.id,
      amount: Math.round(amountToSave * 100) / 100,
  ...
  function getEffectiveGross(inv) {
    const corrections = invoices.filter(c => c.type === 'correction' && c.corrected_doc_id === inv.id);
    if (corrections.length > 0) {
      const latest = corrections.sort((a, b) => new Date(b.created_at) - new Date(a.created_at))[0];
      return parseFloat(latest.gross_amount);
    }
    return parseFloat(inv.gross_amount);
  }
  ...
  const getInvoiceData = (inv) => {
    ...
    return {
      ...
      paid: getPaidAmount(inv),
      dateIssue: inv.issue_date || new Date().toISOString().split('T')[0],
      dateSale: inv.sale_date || inv.issue_date,
      dueDate: inv.due_date,
      seller: { 
        name: shopSettings.name, address: shopSettings.address,
        nip: shopSettings.nip, bankAccount: shopSettings.bankAccount
      },
      buyer: {
        name: inv.buyer_name || inv.customer, nip: inv.buyer_nip || '',
      ...
      items: (inv.items || []).map(it => ({ 
        ...it, 
        vatRate: it.vatRate !== undefined && it.vatRate !== null ? String(it.vatRate) : '0' 
      }))
    };
  };
  ...
  function exportCSV() {
    const csv = 'Nr;Kontrahent;NIP;Netto;VAT;Brutto;Status;Data;Termin\n' + 
      invoices.map(i => `${i.document_number};${i.buyer_name};${i.buyer_nip};${i.net_amount};${i.vat_amount};${i.gross_amount};${i.status};${i.issue_date};${i.due_date}`).join('\n');
  ...
  function renderStatusBadge(inv) {
    ...
    const effective = getEffectiveGross(inv);
    const diff = paid - effective; 
    ...
    <div className="page animate-fadeIn">
            ...
            <tbody>
              {invoices.filter(i => i.type !== 'correction').map(inv => (
                <React.Fragment key={inv.id}>
                  <tr>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                        <button className="btn btn-ghost btn-sm" onClick={() => { setViewInv(inv); setShowView(true); }} title="Podgląd">
                          <FiEye size={14} />
                        </button>
                        
                        <InvoiceDownloadBtn invoiceData={getInvoiceData(inv)} className="btn btn-ghost btn-sm" />
                        
                        <button className="btn btn-ghost btn-sm" onClick={() => sendEmail(inv)} title="Wyślij e-mail">
                          <FiMail size={14} />
                        </button>
                        
                        {/* ZIELONY PTASZEK WPŁATY / ZWROTU - ZAWSZE WIDOCZNY */}
                        <button 
                          className="btn btn-ghost btn-sm" 
                          style={{ color: 'var(--success)' }}
                          onClick={() => {
                            setPaymentTarget(inv);
                            setPaymentAmount(Math.abs(getEffectiveGross(inv) - getPaidAmount(inv)).toFixed(2));
                            setShowPaymentModal(true);
                          }} 
                          title="Zarządzaj wpłatami"
                        >
                          <FiCheckCircle size={14} />
                        </button>
                        
                        {/* EDYCJA / KOREKTA - ZAWSZE WIDOCZNA */}
                        <button className="btn btn-ghost btn-sm" onClick={() => openCorrection(inv)} title="Wystaw korektę">
                          <FiEdit size={14} />
                        </button>
                      </div>
                    </td>
                  ...
                  {invoices.filter(c => c.type === 'correction' && c.corrected_doc_id === inv.id).map(kor => (
      ...
      <Modal isOpen={showCorrectionModal} onClose={() => setShowCorrectionModal(false)} title={`Korekta do: ${correctionTarget?.document_number}`} size="modal-lg" footer={<><button className="btn btn-secondary" onClick={() => setShowCorrectionModal(false)}>Anuluj</button><button className="btn btn-primary" style={{ backgroundColor: 'var(--warning)', borderColor: 'var(--warning)' }} onClick={handleCorrectionSave}>Wystaw Korektę</button></>}>
        <div className="input-group mb-16">
          <label>Przyczyna korekty </label>
          <input className="input" style={{ borderLeft: '4px solid var(--warning)' }} value={correctionReason} onChange={e => setCorrectionReason(e.target.value)} placeholder="np. Zwrot towaru, Pomyłka w cenie..." />
        </div>
        <div className="input-row mb-16">
          <div className="input-group"><label>Kontrahent</label><input className="input" value={form.customer} disabled /></div>
          <div className="input-group"><label>NIP</label><input className="input" value={form.nip} disabled /></div>
        </div>
      ...
      <Modal isOpen={showPaymentModal} onClose={() => setShowPaymentModal(false)} title={`Zarządzanie wpłatami do: ${paymentTarget?.document_number}`} footer={<><button className="btn btn-secondary" onClick={() => setShowPaymentModal(false)}>Anuluj</button><button className="btn btn-success" onClick={handleSavePayment}>Zaksięguj</button></>}>
        <div style={{ marginBottom: '20px', padding: '15px', backgroundColor: 'rgba(0,0,0,0.02)', borderRadius: '8px' }}>
          <div className="flex-between mb-8"><span className="text-muted">Kwota po ew. korektach:</span> <strong style={{fontSize: '1.1em'}}>{paymentTarget && formatCurrency(getEffectiveGross(paymentTarget))}</strong></div>
          ...
          <div className="flex-between">
            <span className="text-muted" style={{ fontWeight: 500 }}>
              {paymentTarget && (getPaidAmount(paymentTarget) > getEffectiveGross(paymentTarget)) ? ' Wymagany zwrot dla klienta:' : 'Oczekuje na przelew:'}
            </span> 
            <strong className={paymentTarget && (getPaidAmount(paymentTarget) > getEffectiveGross(paymentTarget)) ? "text-danger" : "text-warning"}>
              {paymentTarget && formatCurrency(Math.abs(getEffectiveGross(paymentTarget) - getPaidAmount(paymentTarget)))}
            </strong>
          </div>        
        </div>
        <div className="input-group">
          <label>Wpisz kwotę do zaksięgowania (PLN)</label>
          <input className="input" type="number" step="0.01" style={{ fontSize: '1.2em', fontWeight: 'bold' }} value={paymentAmount} onChange={e => setPaymentAmount(e.target.value)} />
        </div>
      </Modal>
      ...
      <Modal isOpen={showView} onClose={() => setShowView(false)} title={`Faktura ${viewInv?.document_number}`} footer={<><InvoiceDownloadBtn invoiceData={viewInv ? getInvoiceData(viewInv) : null} /><button className="btn btn-secondary" onClick={() => setShowView(false)}>Zamknij</button></>}>
        {viewInv && <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[
            ...
            ['Kontrahent', viewInv.buyer_name], 
            ['NIP', viewInv.buyer_nip], 
            ['Netto', formatCurrency(viewInv.net_amount)], 
            ['VAT', formatCurrency(viewInv.vat_amount)], 
            ['Brutto', formatCurrency(viewInv.gross_amount)], 
            ['Data wystawienia', viewInv.issue_date], 
            ['Termin płatności', viewInv.due_date], 
            ['Status', viewInv.type === 'correction' ? 'Rozliczana z oryginałem' : (getPaidAmount(viewInv) - getEffectiveGross(viewInv) > 0.01 ? 'Wymagany zwrot 🔴' : Math.abs(getPaidAmount(viewInv) - getEffectiveGross(viewInv)) <= 0.01 ? 'Opłacona ✅' : viewInv.status === 'overdue' ? 'Po terminie ⚠️' : 'Nieopłacona')],
            viewInv.type === 'correction' ? ['Powód korekty', viewInv.correction_reason] : null
            ...
            <div key={l} className="flex-between" style={{ padding: '6px 0', borderBottom: '1px solid var(--border-light)' }}>
              ...
              <span style={{ fontWeight: 500, color: l === 'Powód korekty' ? 'var(--warning)' : 'inherit' }}>{v}</span>
            </div>
          ))}
        </div>}
      </Modal>
